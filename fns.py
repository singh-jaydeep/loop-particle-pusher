...
def main_iter_loop():  # [Will implement options to choose algorithm, override things like b field, epsilon, dt., etc.]
    ...
    for iters in range(0, c.total_iter):
        ...
        ############################ Main time-step
        if c.PARALLEL:
            (arr_x, arr_y, arr_vx, arr_vy) = time_step_parallel(
                arr_x, arr_y, arr_vx, arr_vy, c.eps, c.dt
            )
        else:
            (arr_x, arr_y, arr_vx, arr_vy) = time_step(
                arr_x, arr_y, arr_vx, arr_vy, c.eps, c.dt
            )

        ############################ Processing which needs to be done during the simulation
        omega_next = np.abs(c.bfield_mag(np.average(arr_x), np.average(arr_y)))
        omega_integrate += 0.5 * c.dt * 1 / c.eps * (omega_curr + omega_next)
...
def init_array():
    ...
    arr_global = np.zeros(
        (c.num_record, c.num_part, 5)
    )  ## for each recording time and particle, store the four values of position (x,y)
    ## and velocity (vx, vy), and integrated omega
    arr_x = c.init_x * np.ones(c.num_part) + c.eps * np.cos(
        arr_theta
    )  ## particles uniformly spaced in a circle of radius eps, centered at (init_x,init_y)
...
def time_step(
    arr_x, arr_y, arr_vx, arr_vy, eps, dt
):  ## Basic Lorentz force equations, using either RK4 or Boris. Specified in
    ## constants.py
    ...
    if c.alg == "RK4":
        ...
        arr_x_next = arr_x + 1 / 6 * dt * (
            arr_x_k1 + 2 * arr_x_k2 + 2 * arr_x_k3 + arr_x_k4
        )
        arr_y_next = arr_y + 1 / 6 * dt * (
            arr_y_k1 + 2 * arr_y_k2 + 2 * arr_y_k3 + arr_y_k4
        )
        arr_vx_next = arr_vx + 1 / 6 * dt * (
            arr_vx_k1 + 2 * arr_vx_k2 + 2 * arr_vx_k3 + arr_vx_k4
        )
        arr_vy_next = arr_vy + 1 / 6 * dt * (
            arr_vy_k1 + 2 * arr_vy_k2 + 2 * arr_vy_k3 + arr_vy_k4
        )
...
@njit(
    parallel=True
)  ## Code with parallel loops. Haven't tested, but should be faster for larger particle numbers
...
def energy(arr_x, arr_y, arr_vx, arr_vy):
    return 0.5 * (np.power(arr_vx, 2) + np.power(arr_vy, 2))


def mag_moment(arr_x, arr_y, arr_vx, arr_vy):
    return np.divide(
        0.5 * (np.power(arr_vx, 2) + np.power(arr_vy, 2)), bfield_mag(arr_x, arr_y)
    )
