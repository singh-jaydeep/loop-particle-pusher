...
def summary_stats(data_array):
    ...
    energy_diff_array = energy_array - np.tensordot(
        np.ones(c.num_record), energy_array[0, :], axes=0
    )
    print("Max energy deviation from t=0 is ", np.max(np.abs(energy_diff_array)))

    magmoment_diff_array = magmoment_array - np.tensordot(
        np.ones(c.num_record), magmoment_array[0, :], axes=0
    )
    print(
        "Max magnetic moment deviation from t=0 is ",
        np.max(np.abs(magmoment_diff_array)),
    )
...
def loop_visualization(data_array):
    ...
    axes[0, 0].scatter(
        arr_rescaledx[int(c.num_record * time1_frac)],
        arr_rescaledy[int(c.num_record * time1_frac)],
    )
    axes[0, 0].set_title(f"time = {time1}")

    axes[1, 0].scatter(
        arr_rescaledx[int(c.num_record * time2_frac)],
        arr_rescaledy[int(c.num_record * time2_frac)],
    )
    axes[1, 0].set_title(f"time = {time2}")

    axes[0, 1].scatter(
        arr_rescaledx[int(c.num_record * time3_frac)],
        arr_rescaledy[int(c.num_record * time3_frac)],
    )
    axes[0, 1].set_title(f"time = {time3}")

    axes[1, 1].scatter(
        arr_rescaledx[int(c.num_record * time4_frac)],
        arr_rescaledy[int(c.num_record * time4_frac)],
    )
...
def harmonic_arrays(
    data_array,
):  ## At present, returns the 0th and 1st harmonics for position and velocity
    arr_x = data_array[:, :, 0]
    arr_y = data_array[:, :, 1]
    arr_vx = data_array[:, :, 2]
    arr_vy = data_array[:, :, 3]
...
def center_POST(
    arr_x, arr_y
):  ## the input arrays are num_record x num_part, ## POST indicates that this computes on the array post simulation
    return np.mean(arr_x, axis=1), np.mean(arr_y, axis=1)


def mean_removed_POST(
    arr_x, arr_y
):  ## POST indicates that this computes on the array post simulation
    arr_center_x, arr_center_y = center_POST(arr_x, arr_y)
    return (
        1 / c.eps * (arr_x - np.tensordot(arr_center_x, np.ones(c.num_part), axes=0)),
        1 / c.eps * (arr_y - np.tensordot(arr_center_y, np.ones(c.num_part), axes=0)),
    )


def nth_harmonic_array(
    arr_input, arr_omega_integrate, n
):  ## here arr_input has shape (num_record,num_part)
    if n != 0:
        arr_return = np.zeros(
            (c.num_record, 2)
        )  ## will return the sin, cos harmonics at each recorded time
        for i in range(0, c.num_record):
            arr_return[i, :] = nth_harmonic_fixedT(
                arr_input[i, :], arr_omega_integrate[i], n
            )
        return arr_return
    ...
    if n == 0:
        arr_return = np.zeros(
            (c.num_record, 1)
        )  ## will return the 0th harmonic (the mean)
        for i in range(0, c.num_record):
            arr_return[i, :] = nth_harmonic_fixedT(
                arr_input[i, :], arr_omega_integrate[i], n
            )
...
def nth_harmonic_fixedT(
    arr_input, omega_integrate, n
):  ## this is integration at fixed time, n = which harmonic (non-negative integer)
    arr_theta = np.arange(0, c.num_part) * c.theta_sep

    if n != 0:
        arr_input_modified_sin = np.multiply(
            arr_input, np.sin(n * (arr_theta + omega_integrate))
        )
        arr_input_modified_cos = np.multiply(
            arr_input, np.cos(n * (arr_theta + omega_integrate))
        )

        arr_return = [0, 0]
        arr_return[0] = (
            1 / (2 * np.pi) * integrate.simpson(arr_input_modified_sin, arr_theta)
        )
        arr_return[1] = (
            1 / (2 * np.pi) * integrate.simpson(arr_input_modified_cos, arr_theta)
        )
